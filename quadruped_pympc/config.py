...
simulation_params = {
    ...
    'mode':                        'waypoints',  # 'human', 'forward', 'random', 'waypoints'

    # Fixed waypoints (x, y) followed in order. Strategy: rotate to face the
    # waypoint, then advance. Used when mode == 'waypoints'.
    'waypoints':                   [(2.0, 0.0), (2.0, 2.0), (0.0, 2.0), (0.0, 0.0)],
    'waypoint_arrival_tol':        0.15,   # m
    'waypoint_yaw_tol':            0.20,   # rad
    'waypoint_lin_vel':            0.18,   # m/s while advancing (max nominal)
    'waypoint_ang_vel':            0.35,   # rad/s while rotating
    'ref_z':                       hip_height,
